...
use \userGroupSelect as UserGroupSelect;
use \printerPPDDialog as PrinterPPDDialog;
use \log as Log;
...
class PrintGeneric extends Plugin
{
    ...
    function __construct(&$config, $dn,$parent_init,$parent)
    {
        ...
        if ($this->dn == "new"){
            ...
            if(!isset($this->config->idepartments[$this->base])){
                MsgDialog::display(_("Internal error"), sprintf(_("Cannot determine a valid department for this object. Setting base to '%s'!"), Session::get('CurrentMainBase')) , WARNING_DIALOG);
        ...
        if(isset($_POST['SavePPD'])){
            ...
            if(count($this->dialog->check())){
                foreach($this->dialog->check() as $msg){
                    MsgDialog::display(_("Error"), $msg, ERROR_DIALOG);
                }
            }else{
                $this->gotoPrinterPPD = array();
    ...
    function remove_from_parent()
    {
        ...
        if($this->initially_was_account){
            ...
            if(!$ldap->count()){
                MsgDialog::display(_("Error"), _("Object is no printer!"), ERROR_DIALOG);
                return;
            }
...
#FIXME FAIscript seams to ununsed within this class... */
    ...
    function DelMember($type,$id)
    {
        ...
        if(!$this->acl_is_writeable("gotoUserPrinter")){
            MsgDialog::display(_("Permission error"), MsgPool::permDelete(_("printer user"), $id), INFO_DIALOG);
            return(FALSE);
        }
    ...
    function AddMember($type,$dn)
    {
        ...
        if(!in_array_ics($type, $types)){
            MsgDialog::display(_("Internal error"), sprintf(_("Illegal member type '%s'!"), $type), ERROR_DIALOG);
            return(FALSE);
        }

        if(!$this->acl_is_writeable("gotoUserPrinter")){
            MsgDialog::display(_("Permission error"), MsgPool::permModify(_("printer user"), $this->dn), INFO_DIALOG);
            return(FALSE);
        }
        ...
        if($ldap->count()){
            ...
            if(isset($attrs[$var][0])){
                ...
                foreach($types as $ctype){
                    ...
                    if(isset(  $this->member[$ctype][$name])){
                        MsgDialog::display(_("Error"), sprintf(_("'%s' is already used!"), $attrs[$var][0]), ERROR_DIALOG);
                        return(FALSE);
                    }
                }
            ...
            MsgDialog::display(_("Error"), sprintf(_("'%s' does not exist!"), $dn), ERROR_DIALOG);
            return(FALSE);
        }
        return(TRUE);
    }
