...
class PpdManager
{
    ...
    function __construct($path)
    {
        ...
        if(is_dir($path)){
            ...
            MsgDialog::display(_("PPD manager error"), sprintf(_("The specified path '%s' does not exist."),$path), ERROR_DIALOG);
            return (false);
        }
    }
    ...
    function saveProperties($ppdFile, $properties)
    {
        if(!is_readable($ppdFile)){
            MsgDialog::display(_("PPD manager error"), sprintf(_("Specified PPD file '%s' cannot be opened for reading."),$ppdFile), ERROR_DIALOG);
        }elseif(!is_writeable(preg_replace("#(^.*/).*$#","\\1",$ppdFile.".tmp"))){
            MsgDialog::display(_("PPD manager error"), sprintf(_("The temporary file '%s' cannot be opened for writing."),$ppdFile.".tmp"), ERROR_DIALOG);
        }else{
