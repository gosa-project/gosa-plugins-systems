...
class SystemManagement extends Management
{
    ...
    function passwordChangeConfirmed()
    {
        ...
        if (!in_array_strict('FAKE_OC_PWD_changeAble', $entry['objectClass'])) {
            ...
            msg_dialog::display(
                _("Error"),
                _("The passwords you've entered as 'New password' and 'Repeated password' do not   match!"),
                ERROR_DIALOG
            );
            return ($smarty->fetch(get_template_path('password.tpl', TRUE)));
        } else {
            ...
            if (!isset($tInfo[$type])) {
                ...
                if (preg_match("/d/", $acl)) {
                    ...
                    if ($info['tabClass'] == "phonetabs") {
                        ...
                        $this->tabObject = new $info['tabClass'](
                            $this->config,
                            $this->config->data['TABS'][$info['tabDesc']],
                            $this->dn,
                            $info['aclCategory'],
                            true,
                            true
    ...
    function handleActivationQueue()
    {
        @DEBUG(
            ...
            __FUNCTION__,
            __FILE__,
            'Entries left: ' . count($this->activationQueue),
            "<b>Handling system activation queue!</b>"
        );
                        ...
                        foreach (array("workgeneric", "termgeneric") as $cls) {
                            if (isset($this->tabObject->by_object[$cls])) {
                                ...
                                @DEBUG(
                                    ...
                                    __FILE__,
                                    $og->dn,
                                    "<b>Calling {$cls}->set_everything_to_inherited()</b>"
                                );
                            }
                        }
                        ...
                        foreach (array("servgeneric", "workgeneric", "termgeneric") as $cls) {
                            if (isset($this->tabObject->by_object[$cls])) {
                                ...
                                @DEBUG(
                                    ...
                                    __FILE__,
                                    $cls,
                                    "<b>Setting auto_activate=TRUE for</b>"
                                );
                            }
                        }
    ...
    protected function saveChanges()
    {
        @DEBUG(
            ...
            __FUNCTION__,
            __FILE__,
            get_class($this->tabObject) . ": " . $this->tabObject->dn,
            "<b>Save</b>"
        );
            ...
            if ($this->tabObject->by_object['ArpNewDevice']->gotoIntegration) {
                ...
                if (count($message)) {
                    ...
                    @DEBUG(
                        ...
                        __FILE__,
                        get_class($this->tabObject) . ": " . $this->tabObject->dn,
                        "<b>Queued for goto activation</b>"
                    );
                    $this->tabObject->save();
        ...
        if ($ldap->count()) {
            ...
            if (count(array_intersect(array('goServer', 'gotoWorkstation'), $attrs['objectClass']))) {
                ...
                @DEBUG(
                    ...
                    __FILE__,
                    $dn,
                    "<b>FAIstate not set to install, this is only done for gotoWorkstation/goServer!</b>"
                );
            }
        }
        return (FALSE);
    ...
    function createSnapshotDialog($action = "", $target = array(), $all = array())
    {
        ...
        foreach ($target as $dn) {
            ...
            if (!empty($dn) && $this->ui->allow_snapshot_create($dn, $pInfo[$type]['aclCategory'])) {
                ...
                msg_dialog::display(
                    _("Permission"),
                    sprintf(_("You are not allowed to create a snapshot for %s."), $dn),
                    ERROR_DIALOG
                );
            }
        }
    ...
    function restoreSnapshotDialog($action = "", $target = array(), $all = array())
    {
        ...
        if (!count($target)) {
            ...
            if (!empty($entry) && $this->ui->allow_snapshot_restore($entry, $cats)) {
                ...
                msg_dialog::display(
                    _("Permission"),
                    sprintf(_("You are not allowed to restore a snapshot for %s."), $entry),
                    ERROR_DIALOG
                );
            }
        } else {
            ...
            if (!empty($dn) && $this->ui->allow_snapshot_create($dn, $pInfo[$type]['aclCategory'])) {
                ...
                msg_dialog::display(
                    _("Permission"),
                    sprintf(_("You are not allowed to restore a snapshot for %s."), $dn),
                    ERROR_DIALOG
                );
            }
        }
    ...
    static function filterSystemDescription($row, $dn, $pid, $state = '', $description = array())
    {
        ...
        switch ($state) {
            case 'locked':
                $rc = "<rowClass:entry-locked/><rowLabel:locked/>";
                break;
            case 'error':
                $rc = "<rowClass:entry-error/><rowLabel:error/>";
                break;
            case 'busy':
                $rc = "<rowClass:entry-busy/><rowLabel:busy/>";
                break;
            case 'warning':
                $rc = "<rowClass:entry-warning/><rowLabel:warning/>";
                break;
        }
