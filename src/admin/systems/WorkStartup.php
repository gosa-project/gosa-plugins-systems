...
# This is disabled as long there is no possiblilty to set these parameters
    ...
    function __construct(&$config, $dn= NULL, $parent= NULL)
    {
        ...
        if(!isset($this->parent->by_object['ogroup'])){
            ...
            if ($ldap->count() > 1) { 
                ...
                MsgDialog::display(_("Warning"), sprintf(_("This system is member of more than one object group, object group inheritence will not work!")."<br><ul>%s</ul>", $str), WARNING_DIALOG); 
            } 
        }
        ...
        if($this->fai_activated && !$this->si_fai_action_failed && $this->si_active){
            ...
            if(!isset($this->cache['SERVERS'][$this->FAIdebianMirror])){
                if(count($this->FAIclass)){
                    MsgDialog::display(_("Error"), sprintf(_("FAI mirror '%s' is not available - setting to mirror 'auto'!"), $this->FAIdebianMirror), ERROR_DIALOG);
                }
            ...
            if(!isset($this->cache['SERVERS'][$this->FAIdebianMirror][$this->FAIrelease])){
                ...
                if(count($this->FAIclass)){
                    MsgDialog::display(_("Error"), sprintf(_("FAI release '%s' is not available on mirror '%s' - setting to release '%s'!"), $this->FAIrelease, $this->FAIdebianMirror,$new_release), ERROR_DIALOG);
                }
    ...
    function execute()
    {
        ...
        if($this->fai_activated){
            if(((isset($_POST['AddClass']))&&(isset($_POST['FAIclassesSel']))) && ($this->acl_is_writeable("FAIclass"))){
                ...
                if((isset($this->FAIclassInfo[get_post('FAIclassesSel')]['FAIpartitionTable']))&&($found>0)){
                    MsgDialog::display(_("Error"), _("There is already a profile containing a partition table in your configuration!") , ERROR_DIALOG);
                }else{
        ...
        if((isset($_POST['gotoShareAdd'])) && isset($_POST['gotoShareSelection']) && ($this->acl_is_writeable("gotoShare"))) {
            ...
            if((!isset($_POST['gotoShareMountPoint']))||(empty($_POST['gotoShareMountPoint']))||(preg_match("/[\|]/i",$_POST['gotoShareMountPoint']))){
                MsgDialog::display(_("Error"), MsgPool::required(_("Mount point")), ERROR_DIALOG);
            }else{
    ...
    function save()
    {
        ...
        if(isset($this->parent->by_object['ogroup'])){
            ...
            MsgDialog::display(_("Fatal error"),
...
another location! */
        ...
        if (!$ldap->success()){
            MsgDialog::display(_("LDAP error"), MsgPool::ldaperror($ldap->get_error(), $this->dn, LDAP_MOD, __CLASS__));
        }
    ...
    function PrepareForCopyPaste($source)
    {
        ...
        $source_o = new WorkstationStartup ($this->config, $source['dn']);
    ...
    function GetHookElements()
    {
        ...
        if(!empty($cmd)){
            ...
            $res2 = trim($res);
            if((!$res)){
                MsgDialog::display(_("Configuration error"), MsgPool::cmdexecfailed("repositoryBranchHook", $cmd), ERROR_DIALOG);
            }elseif(empty($res2)){
                MsgDialog::display(_("Configuration error"), _("'repositoryBranchHook' returned no result!"), ERROR_DIALOG);
            }else{
                $tmp = preg_split("/\n/",$res);
                foreach($tmp as $line){
