...
class PrinterPPDDialog extends Plugin
{
    ...
    function __construct(&$config, $dn= NULL, $ppdfile=NULL )
    {
        ...
        if(empty($this->pathToPPD)){
            MsgDialog::display(_("Configuration error"), MsgPool::invalidConfigurationAttribute("ppdPath"), ERROR_DIALOG);
        }
        ...
        if(!((!is_dir($this->pathToPPD))||(empty($this->pathToPPD)))){ 
            ...
            if(($ppdfile!== NULL)&&(strlen($ppdfile)>0)){
                ...
                if(!file_exists($this->pathToPPD.$this->pathToModified.$ppdfile)){
                    MsgDialog::display(_("PPD error"), sprintf(_("Cannot open PPD '%s'!"), $ppdfile), ERROR_DIALOG);
                }else{
    ...
    function execute()
    {
        ...
        foreach($paths as $path){
            ...
            if(((!is_dir($path))||(empty($path)) || (!$is_r)) && (!@mkdir($path))){
                MsgDialog::display(_("Configuration error"), 
        ...
        if(preg_match("/w/",$acl)){
            if((isset($_FILES['NewPPDFile']))&&(isset($_POST['SubmitNewPPDFile']))){
                ...
                if($file['size'] != 0 ){
                    ...
                    MsgDialog::display(_("PPD error"), MsgPool::incorrectUpload(_("file is empty")), ERROR_DIALOG);
                }
            }
        }
    ...
    function SelectPPD($name)
    {
        ...
        if((!file_exists($AbsoluteSourceName)) || (!is_readable($AbsoluteSourceName))){
            MsgDialog::display(_("PPD error"), MsgPool::cannotReadFile($AbsoluteSourceName), ERROR_DIALOG);
            return;
        }
        ...
        if(!is_dir($AbsoluteDestinationPath.$Vendor)){
            if(!(@mkdir($AbsoluteDestinationPath.$Vendor))){
                MsgDialog::display(_("PPD error"), MsgPool::cannotCreateFolder($AbsoluteDestinationPath.$Vendor), ERROR_DIALOG);
                return(false);
            }
        }
        ...
        if(!$fp){
            MsgDialog::display(_("PPD error"), MsgPool::cannotWriteFile($AbsoluteDestinationPath.$PPDName), ERROR_DIALOG);
            return(false);
        }
    ...
    function AddPPD($_PathOnHdd,$overwrite = FALSE)
    {
        ...
        if((!is_file($_PathOnHdd)) || (!is_readable($_PathOnHdd))){
            MsgDialog::display(_("PPD error"), MsgPool::cannotReadFile($_PathOnHdd), ERROR_DIALOG);
            return(false);
        }
        ...
        if(empty($name) || empty($vendor)){
            MsgDialog::display(_("PPD error"), sprintf(_("Cannot parse PPD '%s'!"), $_PathOnHdd), ERROR_DIALOG);
            return(false);
        }
            ...
            if(preg_match("/".preg_quote($model,'/').".*/i",$key)){
                ...
                if(!$overwrite){
                    if(!copy($_PathOnHdd,$_PathOnHdd."_back")){
                        ...
                        $this->add_later_msg_dialog = new MsgDialog(_("Overwrite existing PPD"),
        ...
        if(!is_dir($this->pathToPPD.$vendor)){
            if(!(@mkdir($this->pathToPPD.$vendor))){
                MsgDialog::display(_("PPD error"), MsgPool::cannotCreateFolder($this->pathToPPD.$vendor), ERROR_DIALOG);
                return(false);
            }
        }
        ...
        if(!$fp){
            MsgDialog::display(_("PPD error"), MsgPool::cannotWriteFile($filename), ERROR_DIALOG);
            return;
        }
        if(empty($contents)){
            MsgDialog::display(_("PPD error"), MsgPool::incorrectUpload(_("file is empty")), ERROR_DIALOG);
            ...
            foreach($this->ppdConfig as $cat => $obj){
                ...
                foreach($obj as $attr => $settings){
                    ...
                    if(($type == "PickOne")||(($type=="Boolean")&&(count($values)>1))){
                        ...
                        MsgDialog::display(_("PPD error"), sprintf(_("PPD type '%s' is not supported!"), $type), ERROR_DIALOG);
                    }
                    $str .= "</td></tr>\n";
                }
            }
    ...
    function removeModifiedPPD()
    {
        ...
        if(file_exists($path)){
            if(is_writeable($path)){
                if(!@unlink($path)){
                    MsgDialog::display(_("PPD error"), MsgPool::cannotDeleteFile($path), ERROR_DIALOG);
                }
            }else{
                MsgDialog::display(_("PPD error"), MsgPool::cannotDeleteFile($path), ERROR_DIALOG);
            }
