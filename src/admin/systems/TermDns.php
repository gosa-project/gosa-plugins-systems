...
class TermDns extends Plugin
{
    ...
    function execute()
    {
        ...
        if (
            $this->config->get_cfg_value("core", "gosaRpcServer") != ""   &&
    ...
    function check()
    {
        ...
        if (
            !empty($this->ipHostNumber) && $this->DNS_is_account &&
        ...
        if (
            !empty($this->macAddress) && $this->dhcp_is_Account &&
        ...
        if ($this->DNS_is_account) {
            ...
            foreach ($this->dnsEntry['RECORDS'] as $name => $values) {
                ...
                if (($values['type'] == "aRecord") && ($values['value'] == $this->ipHostNumber)) {
                    ...
                    $message[] = sprintf(
                        _("The IP address '%s' will be added as 'A Record', this will be done automatically, please remove the record."),
                        $this->ipHostNumber
                    );
                }
    ...
    function save()
    {
        ...
        if ($this->dhcpEnabled && $this->acl_is_writeable("dhcpSetup")) {
            ...
            if ($this->dhcp_is_Account) {
                if (
                    !isset($this->dhcpHostEntry['dhcpHWAddress'][0]) ||
            ...
            if (
                isset($this->dhcpHostEntry['dhcpStatements']) &&
            ...
            if ($this->initial_dhcp_is_Account && $this->dhcp_is_Account) {
                ...
                if (
                    isset($this->dhcpHostEntry['MODIFIED']) && $this->dhcpHostEntry['MODIFIED'] == 1  &&
                    $this->initial_dhcpParentNode == $this->dhcpParentNode
                ) {
                    $attrs = $this->dhcpHostEntry;
                    unset($attrs['dn']);
